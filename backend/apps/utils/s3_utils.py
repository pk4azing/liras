...
def create_activity_s3_folder(cd_id, ccd_id, activity_id):
    s3 = boto3.client('s3')
    today = datetime.utcnow()
    prefix = f"{cd_id}/{ccd_id}/{today.month:02}/{today.day:02}/{activity_id}/"
    try:
        s3.put_object(Bucket=settings.AWS_STORAGE_BUCKET_NAME, Key=prefix)
    except ClientError as e:
        raise Exception(f"Error creating S3 folder: {e}")
    return prefix

def upload_file_to_s3(file_obj, activity):
    s3 = boto3.client('s3')
    today = datetime.utcnow()
    key = f"{activity.cd.id}/{activity.ccd_user.id}/{today.month:02}/{today.day:02}/{activity.id}/{file_obj.name}"

    s3.upload_fileobj(file_obj, settings.AWS_STORAGE_BUCKET_NAME, key)
    return f"s3://{settings.AWS_STORAGE_BUCKET_NAME}/{key}"

def zip_activity_folder(activity):
    s3 = boto3.client('s3')
    today = datetime.utcnow()
    prefix = f"{activity.cd.id}/{activity.ccd_user.id}/{today.month:02}/{today.day:02}/{activity.id}/"
    zip_key = f"{prefix.rstrip('/')}.zip"

    with tempfile.TemporaryDirectory() as tmpdir:
        objects = s3.list_objects_v2(Bucket=settings.AWS_STORAGE_BUCKET_NAME, Prefix=prefix)
        if "Contents" not in objects:
            raise Exception("No files to zip.")

        zip_path = os.path.join(tmpdir, "activity_files.zip")
        with zipfile.ZipFile(zip_path, "w") as zipf:
            for obj in objects["Contents"]:
                file_key = obj["Key"]
                filename = file_key.split("/")[-1]
                tmp_file_path = os.path.join(tmpdir, filename)
                s3.download_file(settings.AWS_STORAGE_BUCKET_NAME, file_key, tmp_file_path)
                zipf.write(tmp_file_path, filename)

        with open(zip_path, "rb") as f:
            s3.upload_fileobj(f, settings.AWS_STORAGE_BUCKET_NAME, zip_key)

    return f"s3://{settings.AWS_STORAGE_BUCKET_NAME}/{zip_key}"

def get_s3_object_content(s3_path):
    s3 = boto3.client('s3')
    bucket = settings.AWS_STORAGE_BUCKET_NAME
    key = s3_path.replace(f"s3://{bucket}/", "") if s3_path.startswith("s3://") else s3_path
    try:
