...
from django.conf import settings
from string import Template
from utils.s3_utils import get_s3_object_content
from django.core.mail import EmailMessage


def send_activity_email(to_email, subject, template_s3_path, context, smtp_config):
    """
    Sends email using an S3-based template and CD-provided SMTP config.

    :param to_email: Recipient email
    :param subject: Email subject
    :param template_s3_path: S3 path to the template (e.g., 'cd1/email_templates/activity_start.txt')
    :param context: dict with keys to replace in template
    :param smtp_config: SMTP details from the CD model
    """
    template = get_s3_object_content(template_s3_path)
    if not template:
        raise Exception("Failed to load email template from S3")

    # Replace placeholders using string.Template
    template_str = Template(template)
    body = template_str.safe_substitute(**context)
    ...
    try:
        with SMTP(smtp_config['host'], smtp_config['port']) as smtp:
            ...
            message = f"From: {smtp_config['from_email']}\nTo: {to_email}\nSubject: {subject}\n\n{body}"
            smtp.sendmail(smtp_config['from_email'], to_email, message)
    except Exception as e:
